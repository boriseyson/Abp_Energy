...
import com.misit.abpenergy.HSE.HazardReport.Service.HazardService
import com.misit.abpenergy.HSE.HazardReport.ViewModel.HeaderViewModel
        ...
        swipeRefreshLayout.setOnRefreshListener(object :SwipeRefreshLayout.OnRefreshListener{
            override fun onRefresh() {
                ...
                GlobalScope.launch(Dispatchers.IO) {
                    async { viewModel.onlineHazard(this@HazardReportActivity, DARI, SAMPAI,disetujui!!,page) }.await()
                    async { viewModel.offlineHazard(this@HazardReportActivity) }.await()
...
//        hazardList?.clear()
//        load("1",DARI, SAMPAI)
        txtTglDari.setOnClickListener(this@HazardReportActivity)
        txtTglSampai.setOnClickListener(this@HazardReportActivity)
        btnLoad.setOnClickListener(this@HazardReportActivity)
        btnNewPost.setOnClickListener(this@HazardReportActivity)
        btnOfflineHazard.setOnClickListener(this@HazardReportActivity)
...
//                                    viewModel.offlineHazard(this@HazardReportActivity,page, DARI, SAMPAI,disetujui!!)
    ...
    private fun hazardViewModel() {
        ...
        viewModel.hazardUser().observe(this@HazardReportActivity,{
            if(it!=null){
                totalHazard.text=it
            }else{
                totalHazard.text="0"
        ...
        viewModel.hazardVerify().observe(this@HazardReportActivity, {
            if(it!=null){
        ...
        viewModel.hazardOffline().observe(this@HazardReportActivity,{
            if(it){
                btnOfflineHazard.visibility = View.VISIBLE
            }else{
                btnOfflineHazard.visibility = View.GONE
            }
        })
        GlobalScope.launch(Dispatchers.IO) {
            async { viewModel.onlineHazard(this@HazardReportActivity, DARI, SAMPAI,disetujui!!,page) }.await()
...
//            startService(connectionService)
        ...
        if(v?.id==R.id.btnOfflineHazard){
            var unsavedIntent = Intent(this@HazardReportActivity,UnsavedHazardActivity::class.java)
            startActivityForResult(unsavedIntent,Constants.UNSAVEDHAZARD)
        }
    }
